...
from .aws_config import get_aws_region, get_kb_data_source_bucket_name, create_boto3_client
from .setup_opensearch_serverless import create_collection, get_collection_arn, verify_collection_exists
...
def create_knowledge_base() -> dict:
    ...
    """
    bedrock_agent_client = create_boto3_client("bedrock-agent")
    kb_name = get_kb_name()
    ...
    try:
        ...
        if not verify_collection_exists():
            logger.info("Creating OpenSearch Serverless collection...")
            create_collection()
        
        collection_arn = get_collection_arn()
        logger.info(f"Using OpenSearch Serverless collection: {collection_arn}")
...
def query_knowledge_base(
    ...
    """
    bedrock_agent_runtime = create_boto3_client("bedrock-agent-runtime")
