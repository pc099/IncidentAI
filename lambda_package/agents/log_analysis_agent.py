...
{{
    ...
    def _invoke_bedrock(self, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"[BEDROCK] Calling invoke_model...")
            response = bedrock_runtime.invoke_model(
                modelId=self.model_id,
                body=json.dumps(body)
            )
            
            print(f"[BEDROCK] Response received, parsing...")
            response_body = json.loads(response['body'].read())
            content = response_body['content'][0]['text']
            print(f"[BEDROCK] Content length: {len(content)} chars")
            
            # Try to parse as JSON
            try:
                result = json.loads(content)
                print(f"[BEDROCK] Successfully parsed JSON response")
                return result
            except json.JSONDecodeError:
                print(f"[BEDROCK] Could not parse as JSON, returning text")
                return {
            ...
            print(f"[BEDROCK ERROR] {str(e)}")  # Add explicit print for Lambda logs
            import traceback
            traceback.print_exc()
            return {
                "error": str(e),
